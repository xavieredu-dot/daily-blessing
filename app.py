...
# --- CONFIGURATION ---
st.set_page_config(page_title="A Daily Blessing",
...
with col2:
    button_clicked = st.button(
        "Open your Daily Verse", use_container_width=True)

if button_clicked:
    # Check if she has already opened it today
    if last_opened == today_str:
        st.markdown(
            '<p class="come-back-text">You have already received your blessing for today. Come back tomorrow! 🌅</p>', unsafe_allow_html=True)
    else:
        ...
        st.markdown(
            f'<div class="verse-box">{verses[index]}</div>', unsafe_allow_html=True)

        # Save today's date so it locks until tomorrow
        set_last_opened_date(today_str)
else:
    st.markdown('<p class="instruction-text">A message from Jesus for you today. ❤️</p>',
                unsafe_allow_html=True)
